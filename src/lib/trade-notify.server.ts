  ...
  return `
    <div style="font-family: -apple-system, Helvetica, Arial, sans-serif; max-width: 480px; margin: 0 auto;">
      ...
      <p style="font-size: 12px; color: #999; margin-top: 24px;">
        You're receiving this because you had a trade on EscrowP2P.
      </p>
    </div>
  `;
}
        ...
        send(
          ...
          `<p style="font-size: 15px; color: #111;">Your trade is complete.</p>
           <p style="font-size: 14px; color: #444; background: #f5f5f5; border-radius: 8px; padding: 12px 14px;">
             You received <strong>${escapeHtml(String(trade.payout_amount))} ${escapeHtml(trade.crypto_type)}</strong> into your EscrowP2P wallet.
           </p>`,
        ),
        send(
          seller,
