...
export const listVerificationRequests = createServerFn({ method: "POST" })
  .middleware([requireSupabaseAuth])
  .inputValidator((input: { status?: "pending" | "approved" | "rejected" | "all" }) => input ?? {})
  .handler(async ({ data, context }) => {
    const { assertAdmin } = await import("@/lib/admin.server");
    await assertAdmin(context.supabase, context.userId);
    const { supabaseAdmin } = await import("@/integrations/supabase/client.server");
    ...
    let query = supabaseAdmin
      ...
      .select(
        "id, document_path, status, note, reviewed_at, created_at, user_id, user:profiles!verification_requests_user_id_fkey(display_name, email)",
      )
      .order("created_at", { ascending: false })
      .limit(200);
    if (data.status && data.status !== "all") query = query.eq("status", data.status);
    ...
    // Signed, short-lived URLs — the bucket is private, so admins need one
    // to actually view the submitted document.
    return Promise.all(
      (rows ?? []).map(async (r) => {
        const { data: signed } = await supabaseAdmin.storage
          .from("verification-docs")
          .createSignedUrl(r.document_path, 600);
        return { ...r, documentUrl: signed?.signedUrl ?? null };
      }),
    );
  });
...
export const reviewVerificationRequest = createServerFn({ method: "POST" })
  ...
  .inputValidator((input: { requestId: string; approve: boolean; note?: string }) => {
    ...
    return { requestId: input.requestId, approve: !!input.approve, note: input.note?.trim() || null };
  })
  .handler(async ({ data, context }) => {
    const { assertAdmin } = await import("@/lib/admin.server");
    await assertAdmin(context.supabase, context.userId);
    ...
    const { data: req, error } = await supabaseAdmin
      ...
      .maybeSingle();
    if (error) throw new Error(error.message);
    if (!req) throw new Error("This request is no longer pending");
...
export const listFeedback = createServerFn({ method: "GET" })
  ...
  .handler(async ({ context }) => {
    ...
    const { data, error } = await supabaseAdmin
      .from("feedback")
      .select("id, message, page_path, created_at, user:profiles!feedback_user_id_fkey(display_name, email)")
      .order("created_at", { ascending: false })
      .limit(200);
    if (error) throw new Error(error.message);
    return data ?? [];
